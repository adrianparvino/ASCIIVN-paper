...
#+NAME: theorem:convex
#+BEGIN_theorem
Given elements \(a, x, y \in \R\) where \(x \leq y\), 
we can define a function \(f_a : \R \to \eR\), defined by \(b \mapsto d(b, a)\).
Let \(x', y'\) be a subderivative of \(f\) at \(x, y\), respectively,
then \(x' \leq y'\).
#+END_theorem
...
Now, given subderivatives \(x', z'\) of \(f_a\) at points, \(x, z \in \R\), respectively, such that \(z > x\),
if \(f z > f x\) then there exists a \(y\), where \(x < y < z\) such that its subderivative, \(y' > 0\).
From theorem [[theorem:convex]] and by transitivity, we can then say that \(z' > 0\).

Therefore, given a sorted list,
#+END_theorem
** Luminosity
...
Applying Theorem [[theorem:summation]], we get the following:
\begin{align}
